...
def parse_youtube(video_search):
    ...
    for index, item in enumerate(items):
        ...
        if(is_following):
            current_search_video["view_count"] = video_view_count
            current_search_video["like_count"] = video_like_count
            current_search_video["dislike_count"] = video_dislike_count
            current_search_video["favorite_count"] = video_favorite_count
            current_search_video["comment_count"] = video_comment_count
            current_search_video["data_published_at_last_video"] = get_channel_founded(published_at)
            current_search_video["title_last_video"] = video_title
            global result_items
    ...
    for root, dirs, files in os.walk("results/"):
        for file in files:
            try:
                ...
                continue
    if(last_item != None):
        dt = pd.read_excel("results/" + last_item)
        return dt, last_index
    else:
        return None, None


def parse_view_count(id):
    video = get_video(id)
    view_count = video["statistics"]["viewCount"]
    return view_count


def parse_statistics(dt):
    current_date = "view_count " + datetime.datetime.utcnow().strftime("%d-%m-%y %H:%M:%S")
    items = dt.to_dict('records')
    for i, item in enumerate(items):
        index = item["video_id"]
        view_count = parse_view_count(index)
        item[current_date] = view_count
        print("Обновление viewCount",
              '[{0:0.2f} %]'.format(i * 100 / len(items)))
    return pd.DataFrame(items)

def save_file(dt, index_filename):
    print("Сохранение результатов")
    dt.to_excel("results/result" + str(index_filename + 1) + ".xlsx", index=False)
    dt.to_csv("results/result"+ str(index_filename + 1) + ".csv", index=False, encoding='utf-8-sig')
...
if __name__ == '__main__':
    ...
    create_path("results/")
    load_api()
    dt, file_name_index = load_file()
    dt_new = parse_statistics(dt)
    save_file(dt_new, file_name_index)
    # parse_view_count("Hewl1vVREik")
